...
def field(text,label):
    vals=[x.strip().strip("*").strip() for x in re.findall(r"^- \**"+re.escape(label.strip("*"))+r"\**[^:：\n]*[:：]\**[ \t]*(.*)$", text, re.M)]
    vals=[x for x in vals if x and not (x.startswith("(") and x.endswith(")"))]
...
table="| 조 | 가게 · 이름 | v1 (9/17) | v2 (10/2 1차 · 10/14 최종 발표) |\n|---|---|---|---|\n"+"\n".join(rows)
s=open(README).read()
