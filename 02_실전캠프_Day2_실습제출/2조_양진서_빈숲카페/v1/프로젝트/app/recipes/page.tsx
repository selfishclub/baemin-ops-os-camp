              ...
              <section className={styles.cupSection} aria-labelledby="cup-title">
                ...
                <div className={styles.cup} aria-label={`${selectedRecipe.name} ${mode} 재료 층`}>
                  {[...selectedVariant.layers].reverse().map((layer, index) => {
                    ...
                    return <div key={`${layer.label}-${index}`} data-tone={layer.tone}>
                      {inlineAdmin ? <>
                        <input aria-label={`재료층 ${sourceIndex + 1} 이름`} value={layer.label} onChange={(event) => updateInlineRecipe((recipe) => ({ ...recipe, variants: { ...recipe.variants, [mode]: { ...recipe.variants[mode]!, layers: recipe.variants[mode]!.layers.map((value, itemIndex) => itemIndex === sourceIndex ? { ...value, label: event.target.value } : value) } } }))} />
                        <input aria-label={`재료층 ${sourceIndex + 1} 값`} value={layer.value} onChange={(event) => updateInlineRecipe((recipe) => ({ ...recipe, variants: { ...recipe.variants, [mode]: { ...recipe.variants[mode]!, layers: recipe.variants[mode]!.layers.map((value, itemIndex) => itemIndex === sourceIndex ? { ...value, value: event.target.value } : value) } } }))} />
                        <select aria-label={`재료층 ${sourceIndex + 1} 색상`} value={layer.tone} onChange={(event) => updateInlineRecipe((recipe) => ({ ...recipe, variants: { ...recipe.variants, [mode]: { ...recipe.variants[mode]!, layers: recipe.variants[mode]!.layers.map((value, itemIndex) => itemIndex === sourceIndex ? { ...value, tone: event.target.value as typeof layer.tone } : value) } } }))}><option value="ice">얼음</option><option value="milk">우유</option><option value="coffee">커피</option><option value="cream">크림</option><option value="topping">토핑</option></select>
                      </> : <><span>{layer.label}</span><strong>{layer.value}</strong></>}
                    </div>
                  })}
                </div>
                {inlineAdmin && <button className={styles.inlineAddButton} type="button" onClick={() => updateInlineRecipe((recipe) => ({ ...recipe, variants: { ...recipe.variants, [mode]: { ...recipe.variants[mode]!, layers: [...recipe.variants[mode]!.layers, { label: "재료층", value: "계량", tone: "milk" }] } } }))}>+ 재료층 추가</button>}
              </section>
        ...
        <span className={styles.cardVisual} data-category={recipe.category} data-has-image={Boolean(primaryImage)}>
          {primaryImage ? <img className={styles.cardImage} src={primaryImage.url} alt="" /> : (
            <span className={styles.miniCup} aria-hidden="true">
              {[...visualLayers].reverse().map((layer, index) => <i key={`${layer.label}-${index}`} data-tone={layer.tone} />)}
            </span>
          )}
