...
const tones: RecipeLayer["tone"][] = ["ice", "milk", "coffee", "cream", "topping"];
...
function textToLayers(value: string): RecipeLayer[] {
  return splitLines(value).map((line) => {
    const [label = "", amount = "", rawTone = "milk"] = line.split("|").map((item) => item.trim());
    const tone = tones.includes(rawTone as RecipeLayer["tone"])
      ? rawTone as RecipeLayer["tone"]
      : "milk";
  ...
  return (
    <article className={styles.variant} data-enabled={Boolean(variant)}>
      ...
      {variant && <>
        ...
        <Field label="음료 층 · 항목 | 값 | 색상"><textarea rows={6} value={layersToText(variant.layers)} onChange={(e) => onChange({ ...variant, layers: textToLayers(e.target.value) })} /></Field>
