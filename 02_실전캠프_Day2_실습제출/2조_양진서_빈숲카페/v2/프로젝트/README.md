...
- 로그인 모드: 사장이 홈의 영역 버튼 아래 "잠그기/열기"를 누른다 (데이터 창고 표 `portal_locks`)
- 둘러보기 모드(로그인 꺼 둠): 서버 환경변수 `LOCKED_SECTIONS=recipes` (쉼표로 여러 개) 를 넣고 다시 배포하면 고정 잠금
- 잠글 수 있는 영역: `recipes`, `training`
