...
const STEPS: { title: string; when: string; items: { text: string; href?: string; label?: string }[] }[] = [
  {
    ...
    items: [
      ...
      { text: "매입 영수증 아래 “영수증 없는 체크카드 결제”는 통장의 체크카드 출금을 매입 영수증과 금액·날짜(±3일)로 맞춰 보고, 영수증이 없는 결제만 모아 보여 줘요. 같은 날 카드 취소, 손익에 안 넣은 개인 결제, 통신비 같은 청구서는 알아서 빠져요. 교통·식대처럼 영수증까지는 필요 없는 건 “필요 없음”을 누르면 다음부터 안 떠요.", href: "/costing", label: "원가율 탭" },
      ...
      { text: "주류 도매상 엑셀을 받기 번거로우면, 주류 판매계산서(영수증) 사진을 폰 AI로 표로 바꿔 “텍스트로 붙여넣기”에 붙여도 돼요. 용기·공병 보증금은 원가에서 빼고, 박스를 병으로 바꿔 병당 원가를 내고, 영수증에 찍힌 채권잔액(월말에 낼 외상)도 기억해 둬요. 붙여넣은 글을 보고 지시문이 알아서 주류용으로 바뀝니다.", href: "/costing", label: "원가율 탭" },
  ...
  },
  {
    ...
    items: [
      ...
      { text: "지난 내역을 찾을 땐 “전체 거래 보기”를 펴고 기간(시작~끝 날짜)과 거래처·분류 글자를 넣으세요. “이 달 전체·최근 7일·지난달” 버튼도 있어요. 찾은 줄 수와 입금·출금 합계가 같이 나와서, 예를 들어 “홈마트”로 찾으면 이 달에 그 마트에 얼마 썼는지 바로 보여요. 기간이 다른 달까지 걸치면 그 달 거래도 같이 찾아 줘요.", href: "/upload", label: "올리기 탭" },
      { text: "정산 탭에서 채널별 수수료율과 “미입금(지남)”을 확인. 규칙이 없는 채널만 주문금액·정산금액을 직접 넣어요.", href: "/channels", label: "정산 탭" },
      { text: "빨간 “입금일 지났는데 없음”(또는 표의 빨간 숫자)을 누르면 어느 날 매출이 언제 들어왔어야 했는지, 근처에 짝 없이 들어온 입금이 있는지, 왜 그럴 수 있는지(규칙보다 일찍·늦게, 다른 카드사로 넣음, 출금 신청 안 함)와 확인할 곳이 나와요.", href: "/channels", label: "정산 탭" },
      ...
      { text: "손익 탭 아래 “세무사용 엑셀” → “엑셀 받기”를 누르면 거래내역·분류별 합계·매출(일별 카드·현금·배달)·매입 영수증이 한 파일로 다운로드 폴더에 저장돼요. 세무사님께는 사장님이 직접 보내요.", href: "/", label: "손익 탭" },
