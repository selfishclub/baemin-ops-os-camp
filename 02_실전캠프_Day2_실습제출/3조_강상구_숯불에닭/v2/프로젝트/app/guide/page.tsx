...
const STEPS: { title: string; when: string; items: { text: string; href?: string; label?: string }[] }[] = [
  {
    ...
    items: [
      ...
      { text: "같은 화면에서 매출 채널 확인. 포스에 카드사별 집계가 있으면 카드 채널을 카드사별로 추가하고 “홀 카드”는 끄기." },
      { text: "정산 탭 → “정산 규칙 등록하기”. 채널마다 ‘매출일 + N영업일’ 또는 ‘주 단위(요일)’. 모르면 “기본 규칙 넣기”로 시작하고, 차이·미입금이 계속 뜨는 채널만 고치면 돼요.", href: "/channels", label: "정산 탭" },
      { text: "원가율 탭 → “품목·레시피 설정”에서 재료(품목)와 기준단가(대략값), 메뉴별 레시피(품목 + g/ml/개). 포스 상품코드를 같이 적어야 판매 자료와 맞춰져요.", href: "/costing", label: "원가율 탭" },
    ],
  },
  {
    title: "매일 마감 때 — 1분",
    when: "장사 끝나고",
    items: [
      { text: "오늘 탭에서 포스 마감정산서의 ‘결제수단별 매출내역’을 옮겨요: 신용카드 → 카드(카드사별로 나눴으면 맨 아래 ‘카드사별 매출내역’ 숫자를 각 칸에), 일반현금·현금영수증 → 홀 현금, 간편결제 → 간편결제. 배달앱은 각 앱 사장님 앱의 오늘 주문금액(수수료 빼기 전).", href: "/today", label: "오늘 탭" },
      { text: "알바는 별칭 고르고 오늘 일한 시간. 어림 인건비와 인건비율이 바로 나와요." },
